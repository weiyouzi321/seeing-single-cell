...
export default function Home() {
  ...
  return (
    <div>
      ...
      <section className="pt-16 pb-12 text-center">
        ...
        {lang === 'zh' ? (
          <p className="text-xl text-gray-500 dark:text-slate-400 max-w-2xl mx-auto mb-10 leading-relaxed">
            ...
            在生物场景中理解数学和算法原理。<br />
            <strong className="text-gray-700 dark:text-slate-200 text-lg">看懂每一行代码背后的逻辑。</strong>
          </p>
        ) : (
          <p className="text-xl text-gray-500 dark:text-slate-400 max-w-2xl mx-auto mb-10 leading-relaxed">
            An interactive visual exploration of scRNA-seq analysis.<br />
            <strong className="text-gray-700 dark:text-slate-200 text-lg">Understand the mathematics and algorithm behind biology.</strong><br />
        ...
        <div className="flex justify-center gap-4">
          ...
          <a
            href="#chapters"
            className="px-6 py-3 rounded-xl border-2 border-gray-200 dark:border-slate-600 text-gray-600 dark:text-slate-300 font-semibold
      ...
      <section id="chapters" className="py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <ChapterCard
            number="07"
            title={t('ch7.title')}
            description={t('ch7.subtitle')}
            concepts={[lang === 'zh' ? '批次效应' : 'Batch Effect', lang === 'zh' ? '锚点' : 'Anchors', lang === 'zh' ? '整合' : 'Integration']}
      ...
      {/* Prerequisites */}
      <section className="py-12 bg-gray-50/50 dark:bg-slate-800/50">
        <div className="max-w-6xl mx-auto px-6">
          <h2 className="text-sm font-semibold tracking-widest uppercase text-gray-400 dark:text-slate-500 mb-2 text-center">
            {lang === 'zh' ? '先修章节' : 'Prerequisites'}
          </h2>
          <p className="text-center text-gray-500 dark:text-slate-400 mb-8 text-sm">
            {lang === 'zh'
              ? '开始单细胞分析之前，建议先掌握以下基础'
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            >
              ...
              <div className="p-6">
                ...
                <h3 className="text-lg font-bold text-gray-800 dark:text-slate-100 mb-2 group-hover:text-indigo-600 transition-colors">
            ...
            >
              ...
              <div className="p-6">
                ...
                <h3 className="text-lg font-bold text-gray-800 dark:text-slate-100 mb-2 group-hover:text-amber-600 transition-colors">
      ...
      {/* How It Works */}
      <section className="py-16 border-t border-gray-100 dark:border-slate-800 mt-8">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-slate-400">{t('home.interactDesc')}</p>
            </div>
            <div>
              <div className="text-3xl mb-3">📖</div>
              <h3 className="font-semibold mb-1">{t('home.read')}</h3>
              <p className="text-sm text-gray-500 dark:text-slate-400">{t('home.readDesc')}</p>
            </div>
            <div>
              <div className="text-3xl mb-3">🧪</div>
              <h3 className="font-semibold mb-1">{t('home.experiment')}</h3>
              <p className="text-sm text-gray-500 dark:text-slate-400">{t('home.experimentDesc')}</p>
            </div>
          </div>
        </div>
      </section>
...
function ChapterCard({
    ...
    <>
      ...
      <div className="flex flex-wrap gap-2">
        {concepts.map((c) => (
          <span
            key={c}
            className="text-xs font-medium text-gray-400 dark:text-slate-500 bg-gray-50 dark:bg-slate-700 px-2.5 py-1 rounded-full"
          >
            {c}
          </span>
        ))}
  ...
  const cls = 'group block p-6 bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm transition-all duration-200' +
    (comingSoon ? ' opacity-60' : ' hover:shadow-md hover:-translate-y-1 hover:border-gray-200 dark:hover:border-slate-600')
